...
function RedeemNavigasiComponent({ category }) {
  ...
  return (
    <div className="flex justify-around md:grid  grid-cols-6 place-items-center r md:w-auto md:grid-cols-2 justify-items-center gap-x-0 md:mx-10  my-1  md:my-0 md:py-10 border border-gray-300 md:border-none md:shadow-xl rounded-lg md:rounded-2xl md:h-fit">
      {data?.map(function (x) {
        return (
          <Link to={"/redeem/" + x.id}>
            <div
      ...
      <Link to="/redeem/lainnya">
        ...
        >
          <div className=" md:my-3 md:mx-3 w-8 md:w-10  basis-1/6  grid grid-cols-1  justify-items-center w-12">
            <div className="w-12">
