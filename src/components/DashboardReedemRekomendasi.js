...
function DashboardReedemRekomendasi() {
  ...
  useEffect(() => {
    axios
      .get(BASE_URL_API + read_cookie("user_cred") + "/items", HEADER_API)
      .then(function (response) {
        console.log(response.data.data);
        setData(
          response.data.data.length > 3
            ? response.data.data.slice(0, 3)
            : response.data.data.slice(0, response.data.data.length)
        );
      })
  ...
  console.log(data);
  return (
    <div className=" grid  grid-cols-1 flex w-full mx-3">
      <div className="grid grid-cols-1 place-content-center bg-orange-500 py-10 px-2 md:p-10 rounded-3xl drop-shadow-xl w-5/5 mt-5 mx-10">
        ...
        <div className="grid grid-cols-3  place-content-center text-center gap-x-1 md:gap-x-2 text-orange-500 text-xs md:text-sm font-semibold px-5">
          {data?.map(function (x) {
            return <ItemRekomendasi value={x.name} />;
          })}
        </div>
      </div>
    </div>
