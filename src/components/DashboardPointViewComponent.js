...
function DashboardPointViewComponent() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        BASE_URL_API + "users/" + read_cookie("user_cred") + "/redeem",
        HEADER_API
      )
      .then(function (response) {
  ...
  return (
    <div className="  bg-white p-10 rounded-3xl drop-shadow-xl  4/5 md:w-full md:mx-8">
      ...
      <div className="grid grid-cols-2 gap-x-2  md:gap-0 ">
        <div className="text-xs md:text-sm text-left ">
          <p>Total Point Masuk</p>
          <h2 className="font-bold md:text-xl text-base">{data.pointIn}</h2>
        </div>
        <div className="text-xs md:text-sm text-right">
          <p>Total Point Keluar</p>
          <h2 className="font-bold md:text-xl text-base">{data.pointOut}</h2>
        </div>
      </div>
    </div>
  );
